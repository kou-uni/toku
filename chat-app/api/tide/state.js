...
const TIDE_CANDIDATES = [
  { id: "kodomo-shokudo", name: "子ども食堂ネットワーク", emoji: "🍱",
    blurb: "ひとり親家庭・困窮家庭の子に、温かい夕食を週2回。全国 4,000 拠点。", vote_share: 0.41 },
  { id: "kokoro-support", name: "心のサポート連絡会", emoji: "🌿",
    blurb: "よりそい電話・チャット相談を匿名で受ける、24時間体制のメンタルヘルス支援。", vote_share: 0.33 },
  { id: "yamadera-mori", name: "山寺と鎮守の森を守る", emoji: "⛩️",
    blurb: "里山・神社林の保全活動。三世代が手入れする、静かな森。", vote_share: 0.26 },
];

const TIDE_PAST = [
  { quarter: "2026 Q1", date: "2026-03-31", amount_usdc: 187.23,
    recipient: "静岡県のフードバンク連合", impact: "47 食の温かい夕食になりました。", photo_emoji: "🍙" },
  { quarter: "2025 Q4", date: "2025-12-31", amount_usdc: 142.08,
    recipient: "よりそいチャット相談センター", impact: "夜の相談員 3 人分の冬の人件費を支えました。", photo_emoji: "🕯️" },
];
...
function pickCity(seed) { return TIDE_CITIES[Math.abs(hash(seed)) % TIDE_CITIES.length]; }
...
function eventToContribution(e) {
  ...
  const labels = {
    LanternSubmitted: "灯火を1枚流した", SessionRecorded: "3分の座を完了",
    GiftCreated: "ギフトを発行", GiftClaimed: "ギフトを受け取った",
    Beat: "世界の鼓動に加わった", TokuMinted: "徳がひとつ積まれた",
  };
  return {
    ts: e.ts, tx: e.tx, city: pickCity(seed),
    label: labels[e.type] || "祈り",
    usdc: Number((perType[e.type] ?? 0.03).toFixed(2)),
...
export default async function handler(req, res) {
  ...
  const candidates = TIDE_CANDIDATES.map((c) => ({
    ...c, votes: Math.round(TIDE_VOTERS_TOTAL * c.vote_share), pct: Math.round(c.vote_share * 100),
  }));
  res.status(200).json({
    pool_usdc: poolUsdc,
